...
# UKVdir = '??/ukmo-nwp/data/ukv-grib/'
year = '2023'
month = '02'
# day = 01
# time = 1200
# UKVdir = f'/badc/ukmo-nwp/data/ukv-grib/{year}/{month}/{day}/{year}{month}{date}{time}_u1096_ng_umqv_Wholesale1.grib'
...
def writenc(var,varname,varunits):
    ...
    """
    ncfile = Dataset(outdir+year+month+'.nc','w')
...
for d in range(nd):
    ...
    for h in tqdm(range(24), desc=f'Processing Day {day}', unit='hour', total=24):
        ...
        hour = str(h).zfill(2)  # zero-fill the string
        grb1 = pygrib.open(f'{UKV_dir}{year}/{month}/{day}/{year}{month}{day}{hour}00_u1096_ng_umqv_Wholesale1.grib')
        grb2 = pygrib.open(f'{UKV_dir}{year}/{month}/{day}/{year}{month}{day}{hour}00_u1096_ng_umqv_Wholesale2.grib')
        
        t = 24*d + h 
        grb = grb1.select()[369+h]
